                ...
                for (int c = 0; c < count; c++)
                {
                    ...
                    if (WorldGen.remixWorldGen)
                    {
                        for (int num963 = 50; num963 < Main.maxTilesX - 50; num963++)
                        {
                            ...
                            {
                                if ((Main.tile[num963, num964].TileType == TileID.Sandstone || Main.tile[num963, num964].TileType == TileID.HardenedSand || Main.tile[num963, num964].TileType == TileID.Sand) && !WorldGen.SolidTile(num963, num964 - 1, false))
                                {
                                    int num965 = num964;
                                    while (num965 < num964 + WorldGen.genRand.Next(4, 7) && Main.tile[num963, num965 + 1].HasTile && (Main.tile[num963, num965].TileType == TileID.Sandstone || Main.tile[num963, num965].TileType == TileID.HardenedSand))
                                    {
                                        Main.tile[num963, num965].TileType = TileID.Sand;
                                        num965++;
                                    }
                                }
                            }
